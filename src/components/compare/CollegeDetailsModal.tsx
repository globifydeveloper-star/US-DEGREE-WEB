...
export default function CollegeDetailsModal({
  ...
  // Parallel data fetching with client caching
  useEffect(() => {
    if (!isOpen || !collegeId) {
      setData(null);
      return;
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [overviewRes, tuitionRes, campusRes, collegeRes] = await Promise.all([
        ...
        const matchedCollege = comparedColleges?.find(c => String(c.id) === String(collegeId));
        ...
        const cipCode = matchedCollege?.cipCode
          || overviewData?.program?.cip_code
          || (collegeId ? universityCipFallbacks[collegeId] : null)
          || "default";
        let outcomesData: any = {};
        try {
          const outcomesRes = await fetch(`${apiUrl}/outcomes/${collegeId}/${cipCode}`);
          if (outcomesRes.ok) {
        ...
        const name = matchedCollege?.name
          || collegeData?.school_name
          || collegeData?.name
          || overviewData?.school_name
          || overviewData?.school?.school_name
          || overviewData?.school?.name
          || (collegeId === "1" ? "Stanford University" : collegeId === "2" ? "UC Berkeley" : "Unknown University");

        const city = collegeData?.city || overviewData?.school?.city || "";
        const state = collegeData?.state || overviewData?.school?.state || "";
        const location = matchedCollege?.location
          || (city && state ? `${city}, ${state}` : (city || state || (collegeId === "1" ? "Stanford, CA" : collegeId === "2" ? "Berkeley, CA" : "Unknown Location")));
        const control = collegeData?.control || overviewData?.school?.control || (matchedCollege?.isPrivate ? "Private" : "Public");
        const type = control.toLowerCase().includes("private") ? "Private" : "Public";

        const logo = matchedCollege?.logo
          || (collegeData?.school_url ? `https://logo.clearbit.com/${new URL(collegeData.school_url.startsWith('http') ? collegeData.school_url : `https://${collegeData.school_url}`).hostname}` : null)
          || (overviewData?.school?.school_url ? `https://logo.clearbit.com/${new URL(overviewData.school.school_url.startsWith('http') ? overviewData.school.school_url : `https://${overviewData.school.school_url}`).hostname}` : null)
          || `https://logo.clearbit.com/google.com`;

        // StatsGrid stats with fallbacks
        const totalStudents = overviewData?.students?.size !== null && overviewData?.students?.size !== undefined
          ? Number(overviewData.students.size)
          : (collegeId === "1" ? 17249 : collegeId === "2" ? 43000 : null);
        ...
        const facultyRatio = rawFacultyRatio
          ...
          : (collegeId === "1" ? "5:1" : collegeId === "2" ? "18:1" : "N/A");
        ...
        const retentionRate = rawRetention !== null && rawRetention !== undefined
          ...
          : (collegeId === "1" ? "98%" : collegeId === "2" ? "97%" : "N/A");

        const programs = overviewData?.school?.program_count !== null && overviewData?.school?.program_count !== undefined
          ? Number(overviewData.school.program_count)
          : (collegeId === "1" ? 20 : collegeId === "2" ? 150 : null);

        const fafsaApplications = overviewData?.students?.fafsa_applications !== null && overviewData?.students?.fafsa_applications !== undefined
          ? Number(overviewData.students.fafsa_applications)
          : (collegeId === "1" ? 2412 : collegeId === "2" ? 5700 : null);
        ...
        const completionRate = rawCompletion !== null && rawCompletion !== undefined
          ...
          : (collegeId === "1" ? "94%" : collegeId === "2" ? "92%" : "N/A");
        ...
        const tuitionInState = tuitionData?.tuition?.tuition_in_state !== null && tuitionData?.tuition?.tuition_in_state !== undefined
          ...
          : (collegeId === "1" ? 56169 : collegeId === "2" ? 14200 : null);
        const tuitionOutOfState = tuitionData?.tuition?.tuition_out_state !== null && tuitionData?.tuition?.tuition_out_state !== undefined
          ...
          : (collegeId === "1" ? 56169 : collegeId === "2" ? 43980 : null);

        // SAT ranges
        const satReadingWriting = overviewData?.admissions?.sat_rw_min !== null && overviewData?.admissions?.sat_rw_max !== null && overviewData?.admissions?.sat_rw_min !== undefined && overviewData?.admissions?.sat_rw_max !== undefined
          ? `${overviewData.admissions.sat_rw_min} - ${overviewData.admissions.sat_rw_max}`
          : (collegeId === "1" ? "690 - 840" : collegeId === "2" ? "660 - 780" : null);

        const satMath = overviewData?.admissions?.sat_math_min !== null && overviewData?.admissions?.sat_math_max !== null && overviewData?.admissions?.sat_math_min !== undefined && overviewData?.admissions?.sat_math_max !== undefined
          ? `${overviewData.admissions.sat_math_min} - ${overviewData.admissions.sat_math_max}`
          : (collegeId === "1" ? "630 - 810" : collegeId === "2" ? "680 - 800" : null);

        const satAverage = overviewData?.admissions?.sat_avg_overall !== null && overviewData?.admissions?.sat_avg_overall !== undefined
          ? String(overviewData.admissions.sat_avg_overall)
          : (collegeId === "1" ? "1500" : collegeId === "2" ? "1380" : null);
        ...
        const acceptanceRate = overviewData?.admissions?.admission_rate !== null && overviewData?.admissions?.admission_rate !== undefined
          ...
          : (collegeId === "1" ? "4.3%" : collegeId === "2" ? "11.4%" : "N/A");
        ...
        const graduates3yr = repaymentRate3YrVal !== null ? `${repaymentRate3YrVal}%` : (collegeId === "1" ? "98%" : collegeId === "2" ? "92%" : "N/A");
        ...
        setCache((prev: any) => ({ ...prev, [collegeId]: resolvedData }));
  ...
  if (data) {
    ...
    coords = trajectoryPoints.map((pt, i) => {
      const x = paddingLeft + (i * (chartWidth - paddingLeft - paddingRight) / (trajectoryPoints.length - 1));
      const targetY = chartHeight - paddingBottom - ((pt.val - minVal) * (chartHeight - paddingTop - paddingBottom) / safeDiff);
          ...
          {isLoading || !data ? (
            ...
            <>
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="bg-[#FAFBFD] border border-slate-100 rounded-2xl p-5 shadow-sm">
                    ...
                    <p className="text-lg font-extrabold text-slate-900 mt-1">{data.acceptanceRate}</p>
                  </div>
                  <div className="bg-[#FAFBFD] border border-slate-100 rounded-2xl p-5 shadow-sm">
                    <p className="text-[10px] font-bold text-slate-400 uppercase">Repayment Rate (3-Year)</p>
                    <p className="text-lg font-extrabold text-slate-900 mt-1">{data.graduates3yr}</p>
                  </div>
                  <div className="bg-[#FAFBFD] border border-slate-100 rounded-2xl p-5 shadow-sm">
                    <p className="text-[10px] font-bold text-slate-400 uppercase">Tuition (EST.) Per Year</p>
                    <p className="text-xs font-extrabold text-slate-900 mt-1 leading-snug">
                      {formatCurrency(data.tuitionInState)} (In-state) / {formatCurrency(data.tuitionOutOfState)} (Out-of-state)
                      <span className="block text-[8px] text-gray-400 uppercase font-bold mt-0.5">per year</span>
                    </p>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="bg-[#FAFBFD] border border-slate-100 rounded-2xl p-5 shadow-sm">
                    <p className="text-[10px] font-bold text-slate-400 uppercase">SAT Reading & Writing</p>
                    <p className="text-base font-extrabold text-slate-800 mt-1">
                      {data.satReadingWriting || <span className="text-slate-400 text-xs italic font-medium">Data Not Available</span>}
                    </p>
                  </div>
                  <div className="bg-[#FAFBFD] border border-slate-100 rounded-2xl p-5 shadow-sm">
                    <p className="text-[10px] font-bold text-slate-400 uppercase">SAT Math</p>
                    <p className="text-base font-extrabold text-slate-800 mt-1">
                      {data.satMath || <span className="text-slate-400 text-xs italic font-medium">Data Not Available</span>}
                    </p>
                  </div>
                  <div className="bg-[#FAFBFD] border border-slate-100 rounded-2xl p-5 shadow-sm">
                    <p className="text-[10px] font-bold text-slate-400 uppercase">SAT Composite (Average)</p>
                    <p className="text-base font-extrabold text-slate-800 mt-1">
                      {data.satAverage || <span className="text-slate-400 text-xs italic font-medium">Data Not Available</span>}
                    </p>
                  </div>
                </div>
              </section>
              ...
              <section className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                  ...
                  <div className="bg-[#FEF9C3]/80 border border-yellow-100 rounded-3xl p-6.5 flex flex-col justify-center min-h-[108px] shadow-sm">
                    ...
                    <p className="text-3xl font-black text-[#16A34A] tracking-tight">
                      {data.salaryYear1 ? formatCurrency(data.salaryYear1 * animProgress) : <span className="text-gray-300 text-xl">No data</span>}
                    </p>
                  </div>
                  ...
                  <div className="bg-[#FEE2E2]/80 border border-red-100 rounded-3xl p-6.5 flex flex-col justify-center min-h-[108px] shadow-sm">
                    ...
                    <p className="text-3xl font-black text-[#16A34A] tracking-tight">
                      {data.salaryYear5 ? formatCurrency(data.salaryYear5 * animProgress) : <span className="text-gray-300 text-xl">No data</span>}
                    </p>
                  </div>
                  ...
                  <div className="bg-[#EEF2FF]/80 border border-indigo-100 rounded-3xl p-6.5 flex flex-col justify-center min-h-[108px] shadow-sm">
                    ...
                    <p className="text-3xl font-black text-[#16A34A] tracking-tight">
                      {data.salaryYear10 ? formatCurrency(data.salaryYear10 * animProgress) : <span className="text-gray-300 text-xl">No data</span>}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-[#FAFBFD] border border-slate-100 rounded-[2rem] p-6 shadow-sm mt-4">
                  <h4 className="text-sm font-bold text-slate-800 mb-4 font-poppins">Salary Trajectory</h4>
                  {hasSalaryData ? (
                      ...
                      >
                        <defs>
                          ...
                          <clipPath id="modalChartClip">
                            <rect x="0" y="0" width={chartWidth * animProgress} height={chartHeight} />
                          </clipPath>
                        </defs>

                        {/* Y Axis Grid Lines */}
                        {[minVal, (minVal + maxVal) / 2, maxVal].map((yVal, i) => {
                          const y = chartHeight - paddingBottom - ((yVal - minVal) * (chartHeight - paddingTop - paddingBottom) / safeDiff);
                          return (
                            <g key={i}>
                              <line
                                x1={paddingLeft}
                        ...
                        {/* Area Fill */}
                        <path d={fillD} fill="url(#modalChartGradient)" clipPath="url(#modalChartClip)" />
                        ...
                        {/* Circle Markers */}
                        {coords.map((coord, i) => {
                          const dotVisible = (chartWidth * animProgress) >= coord.x;
                          return (
                            <circle
                              key={i}
                              cx={coord.x}
                    ...
                    <div className="flex flex-col items-center justify-center py-8 text-center text-slate-400">
                      <p className="text-xs font-semibold">Salary trajectory data is not available</p>
                    </div>
                  )}
                </div>
              </section>
              ...
              <section className="space-y-4">
                ...
                <div className="bg-[#FAFBFD] border border-slate-100 rounded-[2rem] p-6 md:p-8 shadow-sm">
                  <h4 className="text-sm font-bold text-slate-800 mb-6 font-poppins">Gender Distribution</h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-x-12">
                    {/* Student Men */}
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs font-bold text-slate-600">
                        <span>Men (Students)</span>
                        <span>{(data.menStudentsPct * animProgress).toFixed(1)}%</span>
                      </div>
                      <div className="h-2.5 bg-slate-150 rounded-full overflow-hidden">
                        <div className="h-full bg-[#FF5A5A] rounded-full transition-all" style={{ width: `${data.menStudentsPct * animProgress}%` }} />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs font-bold text-slate-600">
                        ...
                        <span>{(data.menFacultyPct * animProgress).toFixed(1)}%</span>
                      </div>
                      <div className="h-2.5 bg-slate-150 rounded-full overflow-hidden">
                        <div className="h-full bg-[#FF5A5A] rounded-full transition-all" style={{ width: `${data.menFacultyPct * animProgress}%` }} />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs font-bold text-slate-600">
                        ...
                        <span>{(data.womenStudentsPct * animProgress).toFixed(1)}%</span>
                      </div>
                      <div className="h-2.5 bg-slate-150 rounded-full overflow-hidden">
                        <div className="h-full bg-[#EC4899] rounded-full transition-all" style={{ width: `${data.womenStudentsPct * animProgress}%` }} />
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs font-bold text-slate-600">
                        ...
                        <span>{(data.womenFacultyPct * animProgress).toFixed(1)}%</span>
                      </div>
                      <div className="h-2.5 bg-slate-150 rounded-full overflow-hidden">
                        <div className="h-full bg-[#EC4899] rounded-full transition-all" style={{ width: `${data.womenFacultyPct * animProgress}%` }} />
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="pt-6 border-t border-slate-200 shrink-0 flex flex-col sm:flex-row items-center justify-center gap-2 text-slate-400">
                ...
                <p className="text-[11px] md:text-xs text-slate-500 font-medium tracking-wide text-center leading-normal select-none">
                  College details shown here are intended for quick comparison purposes. For complete institution and program information, use the search experience on the landing page to explore individual colleges and programs.
                </p>
              </div>
            </>
          )}
