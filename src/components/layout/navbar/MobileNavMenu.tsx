...
export default function MobileNavMenu({
  ...
  return (
    <div
      className={`absolute top-[70px] left-0 w-full grid lg:hidden transition-[grid-template-rows] duration-300 ease-in-out z-50 ${
        isOpen ? "grid-rows-[1fr]" : "grid-rows-[0fr]"
      }`}
    >
      <div
        className={`overflow-hidden bg-white border-b border-[#f0f2f5] shadow-lg transition-opacity duration-300 ease-in-out ${
          isOpen ? "opacity-100" : "opacity-0"
        }`}
      >
        <div className="flex flex-col p-6 gap-6 text-[#4b5563] font-medium">
          {NAV_LINKS.map((link) =>
